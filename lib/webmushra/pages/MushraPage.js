...
MushraPage.prototype.render = function (_parent) {
  ...
  for (i = 0; i < conditions.length; ++i) {
    td = $("<td class='spaceForSlider'> \
      <span><input type='range' name='"+conditions[i].getId()+"' class='scales' value='5' min='1' max='5' data-vertical='true' data-highlight='true' style='display : inline-block; float : none;'/></span> \
    </td>");
...
MushraPage.prototype.renderCanvas = function(_parentId) {
  ...
  var xDrawingStart = $('#spaceForScale').offset().left - $('#spaceForScale').parent().offset().left + canvasContext.measureText("100 ").width * 1.5 + canvas.width * 0.2;
  ...
  canvasContext.fillText('excellent', xTextScoreRanges, YfirstLine);
  canvasContext.fillText('good', xTextScoreRanges, prevScalesHeight * 0.25 + YfirstLine);
  canvasContext.fillText('fair', xTextScoreRanges, prevScalesHeight * 0.5 + YfirstLine);
  canvasContext.fillText('poor', xTextScoreRanges, prevScalesHeight * 0.75 + YfirstLine);
  canvasContext.fillText('bad', xTextScoreRanges, prevScalesHeight * 1 + YfirstLine);
