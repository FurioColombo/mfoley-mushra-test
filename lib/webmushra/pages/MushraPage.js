...
MushraPage.prototype.render = function (_parent) {
  ...
  for (i = 0; i < conditions.length; ++i) {
    td = $("<td class='spaceForSlider'> \
      <span><input type='range' name='"+conditions[i].getId()+"' class='scales' value='5' min='0' max='5' data-vertical='true' data-highlight='true' style='display : inline-block; float : none;'/></span> \
    </td>");
...
MushraPage.prototype.renderCanvas = function(_parentId) {
  ...
  canvasContext.fillText("5", xTextScoreRanges, YfirstLine);
  canvasContext.fillText("4", xTextScoreRanges, prevScalesHeight * 0.2 + YfirstLine);
  canvasContext.fillText("3", xTextScoreRanges, prevScalesHeight * 0.4 + YfirstLine);
  canvasContext.fillText("2", xTextScoreRanges, prevScalesHeight * 0.6 + YfirstLine);
  canvasContext.fillText("1", xTextScoreRanges, prevScalesHeight * 0.8 + YfirstLine);
